...
import { TniProduceLimitType, TniSocketType, TniTraversalConsumptionPolicy } from 'raw/data-format5-spec.js';
...
function _getDeepDiff(object, base) {
    return _.transform(object, (result, value, key) => {
        if (!_.isEqual(value, base[key])) {
            result[key] = (_.isObject(value) && _.isObject(base[key]))
                ? _getDeepDiff(value, base[key])
                : value;
        }
    });
}
...
function getSumCombinations(numbers, maxLength) {
    const uniqueSums = new Map();
    function generate(currentCombo, currentSum) {
...
let MyDevice = class MyDevice extends LitElement {
    ...
    get _dropdown_items_satas_templates() {
        if (this.__dropdown_items_satas_templates_data !== this._data) {
            ...
            if (this._data) {
                for (const plug_id in this._data.plugs) {
                    if (plug_id == "components/removables/usb_stick.tscn")
                        continue;
    ...
    _getSataCombinations(sata_port_count) {
    ...
    _getProgramsRequirements(programs) {
        let programs_cpu = 0;
        let programs_mem = 0;
        let programs_size = 0;
    ...
    render() {
                        ...
                        if (!input.value) {
                            ...
                            this.requestUpdate();
								...
								<wa-number-input
									...
									@input=${(e) => {
                        logic_controller.installed_sto = Number.parseInt(e.target.value ?? "0") - satas_size;
                        this.requestUpdate();
                    }}
									@blur=${(e) => {
                        const input = e.target;
        ...
        if (this._data) {
            ...
            if (this.device_data_is_original || (programs_cpu <= 0 && programs_mem <= 0 && programs_size <= 0)) {
                for (const device_id in this._data.devices) {
                    const device = this._data.devices[device_id];
                    if (!device.logic_controller)
                        continue;
                    dropdown_templates.push(dropdownItemTemplate(device_id, IRRELEVANT_DEVICES.has(device_id) ? "var(--wa-color-gray)" : "", [
        ...
        return html `
			<wa-card class="card-header">
				<div slot="header" class="flex-wrap-gap" style="align-items: center;">
					<my-combobox size="l" style="flex-grow: 1;" value=${this.device_id ?? ""} input_value=${this.device_name ?? ""}
						@my-value-confirm=${(e) => { this.device_id = e.detail.value ?? null; }}
					>
            ...
            if (this.previousElementSibling) {
                this.previousElementSibling?.requestUpdate();
            ...
            if (this.nextElementSibling) {
                this.nextElementSibling?.requestUpdate();
    ...
    _generateTemplatesForPeripherals(logic_controller, custom_data) {
        ...
        for (let port_index = 0; port_index < logic_controller.ports.length; port_index++) {
            ...
            if (port.type != TniSocketType.SATA35_SLOT)
                continue;
            templates.push(html `
				<my-combobox
    ...
    _getSataPorts(logic_controller) {
				...
				<tr>
					<th style="width: 100%;">
						<my-combobox slot="header" style="flex-grow: 1;" value=${program_id}
							@my-value-confirm=${(e) => { programs[i] = e.detail.value ?? ""; this.requestUpdate(); }}
						>
